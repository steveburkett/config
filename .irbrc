...
def extend_console(name, care = true, required = true)
  if care
    ...
    $console_extensions << "#{ANSI[:RESET]}#{ANSI[:LGRAY]}#{name}#{ANSI[:RESET]}"
  end
rescue LoadError
...
def clear
  system 'clear'
  (defined? Rails) ? "Rails environment #{Rails.env}" : "No Rails environment"
end
...
puts "#{ANSI[:WHITE]}~> Extensions:#{ANSI[:RESET]} #{$console_extensions.join(' ')}#{ANSI[:RESET]}"
